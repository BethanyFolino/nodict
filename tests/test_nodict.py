...
class TestNode(unittest.TestCase):
    ...
    def test_node_instance(self):
        """Regression test for checking if we can create an instance"""
        nd = self.module.Node("Kevin")
        self.assertIsNotNone(nd, "Unable to create a Node instance")

    def test_comparison_eq(self):
        """Check if comparisons work"""
        wallace = self.module.Node("Wallace", 5)
        grommit = self.module.Node("Grommit", 5)
        other_wallace = self.module.Node("Wallace", 9)
...
class TestNoDict(unittest.TestCase):
    ...
    def test_add(self):
        ...
        d.add("Jake", "Toy")
        expected_node1 = self.module.Node("Ralphie", "BB gun")
        expected_node2 = self.module.Node("Jake", "Toy")
        self.assertEqual(len(d.buckets[0]), 2,
                         'Bucket does not allow for multiple nodes!')
        actual_node1 = d.buckets[0][0]
        actual_node2 = d.buckets[0][1]
        self.assertEqual(actual_node1, expected_node1)
        self.assertEqual(actual_node2, expected_node2)
    ...
    def test_get(self):
        ...
        try:
            ...
            self.fail("Existing Node not found!")
    ...
    def check_no_duplicates_bucket(self, method='add'):
        """Check that duplicates are not allowed"""
        # New Node of same name: new value should overwrite previous value
        d = self.module.NoDict(1)
        setitem = d.add
        ...
        setitem("Zeppo", 56)
        # All buckets should be empty, except one
        self.assertEqual(len(d.buckets[0]), 3,
                         "Node added too many times!")

    def test_no_duplicates_add(self):
        self.check_no_duplicates(method='add')
        self.check_no_duplicates_bucket(method='add')

    def test_no_duplicates_setitem(self):
        self.check_no_duplicates(method='setitem')
        self.check_no_duplicates_bucket(method='setitem')
